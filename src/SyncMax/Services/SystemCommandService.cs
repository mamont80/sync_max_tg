...
public sealed class SystemCommandService
{
    ...
    public async Task<bool> TryHandleAsync(MessengerType messenger, string userId, string text, CancellationToken ct)
    {
        ...
        switch (command.ToLowerInvariant())
        {
            ...
            case "/clear":
                ...
                return true;
            case "/app":
                await HandleOpenAppAsync(messenger, userId, ct);
                return true;
            default:
                return false;
        }
    ...
    /// <summary>
    /// /app — присылает сообщение с кнопкой, открывающей мини-приложение. Кнопку рисует
    /// каждый клиент по-своему (см. <see cref="IMessengerApiClient.SendMiniAppButtonAsync"/>);
    /// если адрес приложения не настроен, придёт просто текст.
    /// </summary>
    private async Task HandleOpenAppAsync(MessengerType messenger, string userId, CancellationToken ct)
    {
        ...
        await client.SendMiniAppButtonAsync(
            userId,
            "⚙️ Управление связками чатов, подпиской и настройками — в приложении.",
            "Открыть приложение",
            ct);
    }
    ...
    private async Task HandleClearAsync(MessengerType messenger, string userId, CancellationToken ct)
    {
        ...
        await _linking.ResetAndNotifyAsync(messenger, userId, ct);

        if (counterpartUserId is not null)
        {
            var counterpartMessenger = Other(messenger);
            await _linking.ResetAndNotifyAsync(counterpartMessenger, counterpartUserId, ct);
        }
